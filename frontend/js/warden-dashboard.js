...
function loadOccupancyChart() {
  ...
  const occupiedBeds = rooms.reduce((total, room) => total + room.occupiedBeds.length, 0);
  ...
  chartContainer.innerHTML = `
    ...
    <div class="occupancy-details">
      ...
      <div class="room-types">
        <div class="room-type">
          <div class="room-type-label">Standard</div>
          <div class="room-type-count">${rooms.filter(r => r.type === 'Standard').length} rooms</div>
        </div>
        <div class="room-type">
          <div class="room-type-label">Premium</div>
          <div class="room-type-count">${rooms.filter(r => r.type === 'Premium').length} rooms</div>
        </div>
      </div>
    </div>
  `;
...
function loadRoomsPage() {
  ...
  if (rooms.length === 0) {
    ...
    // Add rooms to grid
    rooms.forEach(room => {
      const occupancyRate = (room.occupiedBeds.length / room.capacity) * 100;
        ...
        <div class="room-body">
          <div class="room-info">
            ...
            <div class="info-item">
              ...
              <span class="value">${room.occupiedBeds.length} beds</span>
            </div>
          </div>
    ...
    const roomButtons = roomsList.querySelectorAll('.view-room-btn');
    roomButtons.forEach(button => {
      button.addEventListener('click', () => {
        const roomId = button.getAttribute('data-room-id');
        // In a real app, this would show room details
        showNotification('Room details view would be shown in a real application.', 'info');
      });
    });
  }
  ...
  if (students.length === 0) {
    ...
    students.forEach(student => {
      ...
      row.innerHTML = `
        ...
        <td>
          <button class="btn btn-sm btn-outline view-student-btn" data-student-id="${student.id}">
...
function loadMaintenancePage() {
  ...
  // Sort requests by date (newest first) and status
  requests.sort((a, b) => {
  ...
  if (requests.length === 0) {
    ...
    requests.forEach(request => {
      ...
      row.innerHTML = `
        ...
        <td>
          <button class="btn btn-sm btn-outline view-request-btn" data-request-id="${request.id}">
            <i class="fas fa-eye"></i>
          </button>
          <button class="btn btn-sm btn-primary update-request-btn" data-request-id="${request.id}">
    ...
    const viewButtons = maintenanceList.querySelectorAll('.view-request-btn');
    viewButtons.forEach(button => {
      button.addEventListener('click', () => {
    ...
    const updateButtons = maintenanceList.querySelectorAll('.update-request-btn');
    updateButtons.forEach(button => {
      button.addEventListener('click', () => {
        const requestId = button.getAttribute('data-request-id');
        openUpdateMaintenanceModal(requestId);
      });
    });
  }
}
